...
# can read bootstrap state. Holding token values directly in state —
# even with `sensitive = true` — leaks them at that layer. Per-secret
# accessor IAM moves the authorization check off the bucket and onto
# each secret: RO SA reaches only the RO token, RW SA only the RW.
